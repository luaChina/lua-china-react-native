...
export default class PostListComponent extends Component {
  ...
  // 前缀下划线，代表私有方法
  _renderRow(row) {
    return (<PostItem key={row.id} navigate={this.props.navigation.navigate} row={row}/>);
  }
  ...
  _fetchData(p = 1, refresh = false) {
    ...
    ApiService.get(config.api.posts, {p})
      .then((response) => {
        let items;
        items = postCachedResults.items.concat(response.data.data);
        postCachedResults.items = items;
        postCachedResults.total = response.data.total;
        postCachedResults.nextPage++;
        setTimeout(() => {
          if (refresh) {
            // 先清空
            this.setState({
  ...
  _fetchMoreData() {
    ...
    this._fetchData(postCachedResults.nextPage);
  }

  _hasMore() {
    return postCachedResults.total > postCachedResults.items.length;
  }
